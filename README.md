...
- 已知坑(已修正)：**例外是 00991A(復華)**，它的 ZIP 日期本來就是交易基準日，
  不能平移(`NO_SHIFT_TICKERS`)。原本一律平移導致它的歷史持股整體早了一個交易日，
  「個股跨 ETF 比較」走勢圖上它的買賣進出點都標在前一天。判斷方法：拿各檔 ETF 的
  NAV 日報酬去對「前一日持股 x 個股收盤價變化」算相關係數，日期對齊時約 0.97，
  錯位一天就掉到 0.1 左右；新增 ETF 進 ZIP 匯入前建議先用同樣方式驗證日期定義。
- 若某檔 ETF 沒有出現在 ZIP 裡（例如本專案的 00988A），會直接略過，維持原本的
  即時爬蟲資料不受影響。

## 補歷史缺口 / 修正日期錯位 (`scripts/backfill_etf.py`)

排程漏跑(電腦沒開機、GitHub 排程延遲)會讓 ETF 資料出現缺口，但四家投信的 API 都能
指定查詢日期，所以缺的交易日可以事後補回：

```powershell
python scripts/backfill_etf.py --dry-run          # 先看會補哪些日期
python scripts/backfill_etf.py                    # 補 2026-01-02 ~ 最新之間所有缺的交易日
python scripts/backfill_etf.py --tickers 00981A --start 2026-08-01
python scripts/backfill_etf.py --replace 00981A:2026-08-25   # 該區間先刪掉再重抓(修正日期錯位的列)
```

- 「查詢日」是公告日：回傳的是查詢日**之前**某個交易日的資料(多數投信是前一個交易日，
  統一全球 00988A 因海外資產估值時差落後**兩個**交易日)，所以腳本會從目標日之後的
  交易日依序當查詢日，直到回應的資料日期剛好等於目標日才採用。交易日曆取自「至少 50 檔個股
  有收盤價」的日期，國定假日不會被誤判成缺口。
- 每次執行前會把資料庫備份到系統暫存資料夾(不在 `data/` 裡，避免被 commit)。
- 已知坑(已修正)：
  - **統一投信(00981A/00403A/00988A)的資料日期整批早一天**。它的回應日期有兩種格式，
    其中 `/Date(毫秒)/` 是「台灣時間午夜」，`parse_any_date` 原本當成 UTC 解析，
    所以週一變成週日、其他日子早一天，資料庫還因此出現週日的列。已改成以 UTC+8 換算，
    並用 `--replace` 重抓 2026-08-25 之後的資料。
  - **歷史 ZIP 在 2026-01-15 → 01-27 之間沒有公告資料**(缺 7 個交易日)，而匯入時
    「日期 = 前一筆公告日」的平移規則遇到缺口會把缺口後第一筆標成缺口前的日期，
    造成 01-15 這一列其實是 01-26 的資料。已刪除重抓並補齊 1/15–1/26。
  - 驗證日期是否對齊的方法：NAV 日報酬 vs「前一日持股 x 個股收盤價變化」的相關係數，
    對齊時 0.95 以上，錯位一天約 0.1。目前 7 檔台股型 ETF 各時期都是對齊的。
- 無法補的缺口(來源端本來就沒公布)：00988A 的 2026-01-19、02-10、05-25(API 回傳的日期序列
  直接跳過這幾天)，另外 07-08 那天來源端只有淨值、沒有持股明細；00988A 最新一天會比其他檔
  晚兩個交易日才有資料，屬正常現象。
